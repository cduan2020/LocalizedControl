...
if nargin <= 6
    ...
    addParameter(ip, 'N', N, @(x) isnumeric(x) && isscalar(x));
    addParameter(ip, 'm0', m0, @(x) isnumeric(x) && isscalar(x) && (x > 0 && x < N));
    addParameter(ip, 'm', m, @(x) isnumeric(x) && isscalar(x) && x <= ip.Results.m0);
